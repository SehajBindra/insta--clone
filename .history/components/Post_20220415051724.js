...
function Post({ id, username, userImg, img, caption }) {
  ...
  const [bookmark , setBookamrk ] = useState();
    ...
    <div className="  bg-white my-7 border rounded-sm">
      ...
      {session && (
        <div className=" flex justify-between px-4 pt-4">
          ...
          <BookmarkIcon onClick={() => bookmark ? setBookamrk(false): setBookamrk(true)} className="btn"
           />
           { bookmark ? ( <BookmarkIcon className=" h-7" htmlColor="#000000" />
         ) : (
          <BookmarkIcon className=" h-7 text-black"/>
         )
  }
        </div>
      )}
